...
def get_arg_parser():
    ...
    group.add_argument("-H", "--HELP", action="store_true", help="show this help message for MPyC and exit")
    group.add_argument("-h", "--help", action="store_true", help="show help message for this MPyC program (if any)")

    group = parser.add_argument_group("MPyC configuration")
    group.add_argument("-C", "--config", metavar="ini", help="use ini file, defining all m parties")
    group.add_argument(
        "-P",
        type=str,
        dest="parties",
    ...
    group.add_argument("-M", type=int, metavar="m", help="use m local parties (and run all m, if i is not set)")
    group.add_argument("-I", "--index", type=int, metavar="i", help="set index of this local party to i, 0<=i<m")
    group.add_argument("-T", "--threshold", type=int, metavar="t", help="threshold t, 0<=t<m/2")
    group.add_argument("-B", "--base-port", type=int, metavar="b", help="use port number b+i for party i")
    group.add_argument("--ssl", action="store_true", help="enable SSL connections")
    ...
    group.add_argument(
        "--output-windows", action="store_true", help="screen output for parties 0<i<m (only on Windows)"
    )
    group.add_argument("--output-file", action="store_true", help="append output of parties 0<i<m to party{m}_{i}.log")
    group.add_argument("-f", type=str, default="", help="consume IPython's -f argument F")
